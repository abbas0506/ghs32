        ...
        @if (Auth::user()->section)
            ...
            <div class="flex flex-wrap items-center gap-4 mt-4">
                ...
                <div class="flex flex-wrap items-center gap-4 text-sm">
                    <a href="{{ route('teacher.test.section.result.print', [$test, Auth::user()->section]) }}"
                        target="_blank" class="link">Result Summary</a>|
                    <a href="{{ route('teacher.test.section.positions.print', [$test, Auth::user()->section]) }}"
                        target="_blank" class="link">Class Positions</a>|
                    <a href="{{ route('teacher.test.section.report-cards.print', [$test, Auth::user()->section]) }}"
                        target="_blank" class="link">Result Cards</a>
                </div>
            </div>
                ...
                <tbody>
                    @foreach ($testAllocations as $testAllocation)
                        <tr class="tr @if ($testAllocation->hasBeenSubmitted()) submitted @endif">
                            ...
                            <td>
                                @if ($testAllocation->hasBeenSubmitted())
                                    <a href="{{ route('shared.test-allocation.result.print', $testAllocation) }}"
                                        target="_blank"><i class="bi-printer"></i></a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
