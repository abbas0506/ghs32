...
@section('page-content')
    ...
    <div class="bread-crumb">
        <a href="/">Home</a>
        <div>/</div>
        <a href="{{ route('admin.class-schedule') }}">Allocations</a>
        <div>/</div>
        <div>Edit</div>
    </div>
    ...
    <div class="md:w-3/4 mx-auto mt-6 bg-white md:p-8 rounded">
        ...
        <div class="flex justify-between">
            ...
            <form
                action="{{ route('admin.section.lecture.schedule.destroy', [$allocation->section, $allocation->lecture_no, $allocation]) }}"
                method="POST" onsubmit="return confirmDel(event)">
                @csrf
                @method('DELETE')
                <button type="submit"
                    class="flex justify-center items-center w-12 h-12 rounded-full border hover:bg-slate-100">
        ...
        <form
            action="{{ route('admin.section.lecture.schedule.update', [$allocation->section, $allocation->lecture_no, $allocation]) }}"
            method='post' class="w-full grid gap-6" onsubmit="return validate(event)">
            @csrf
            @method('PATCH')
            <div>
                <label>Subject</label>
