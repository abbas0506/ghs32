...
<div class="custom-container">
    ...
    <div class="flex items-center flex-wrap justify-between mt-8">
        <div class="flex items-center flex-wrap gap-4">
            <form action="{{route('library.print.rack-books.qr', $rack)}}" method="get" class="flex items-center flex-wrap gap-x-4">
                @csrf
        ...
        <table class="table-fixed w-full">
            ...
            <tbody>
                ...
                <tr class="tr">
                    ...
                    <td>{{$sr++}}</td>
                    <td class="text-left">
                        <a href="{{ route('library.rack.books.show', [$rack,$book]) }}" class='link'>{{$book->title}}</a>
                    ...
                    <td>
                        <div class="flex items-center justify-center">
                            <a href="{{route('library.rack.books.edit',[$rack, $book])}}"><i class="bx bx-pencil text-green-600"></i></a>
                            <span class="text-slate-300 px-2">|</span>
                            <form action="{{route('library.rack.books.destroy',[$rack, $book])}}" method="post" onsubmit="return confirmDel(event)">
                                @csrf
                                @method('DELETE')
